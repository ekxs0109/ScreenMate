    ...
    <section className="flex flex-col gap-3">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-sm text-slate-500 dark:text-slate-400">
        <div className="flex items-center gap-2">
          <span className="font-medium text-slate-700 dark:text-slate-300">{copy.roomLabel}:</span>
          <span className="font-mono bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-md">{roomId || copy.waitingRoomId}</span>
        </div>
        
        <div className="flex items-center">
          {roomState === "closed" ? (
            <span className="text-red-500 dark:text-red-400">{copy.hostEndedRoom}</span>
          ) : sourceState === "recovering" ? (
            <span className="text-amber-500">{copy.hostReconnectingVideoSource}</span>
          ) : sourceState === "missing" ? (
            <span>{copy.waitingForVideoSource}</span>
          ) : status === "connected" ? (
        ...
        {!stream && (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="flex flex-col items-center gap-3 text-slate-400">
              ...
              <p className="text-sm font-medium">{copy.waitingForVideoStream}</p>
            </div>
          </div>
        )}
        ...
        {needsInteraction && stream && (
          ...
          >
            <div className="flex flex-col items-center gap-3 bg-slate-800/80 px-6 py-4 rounded-2xl shadow-xl transform transition-transform group-hover:scale-105 border border-slate-700">
              ...
              <span className="font-semibold text-lg">{copy.clickToUnmute}</span>
            </div>
          </button>
        )}
      </div>
